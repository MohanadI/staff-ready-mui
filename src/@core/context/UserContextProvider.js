...
import { useDocumentImproveAPI } from "../../api/documentimprove";
...
function UserContextProvider(props) {
    ...
    const getUserData = useCallback(async () => {
        let userData;
        await axios.get(`/StaffReady/v10/api/account/user`)
            .then(res => {
                userData = res.data
                setUser(userData);
            })
            .catch(error => {
                if (error.response.status === 404) { // this should only occur if the user is an admin
                    axios.get(`/StaffReady/v10/api/account/userId`)
                        .then(res => {
                            userData = { name: res.data };
                            setUser(userData);
                        })
                    axios.get(`/StaffReady/v10/api/user-access/is-admin`)
                        .then(res => {
    ...
    const getUserPermissions = useCallback(async () => {
        await axios.get(`/StaffReady/v10/api/authorization/permissions`)
            .then(res => {
